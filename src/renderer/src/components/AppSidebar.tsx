...
import { getAccessibleSidebarItems } from '../config/navigation'
...
export const AppSidebar: React.FC<AppSidebarProps> = ({
  ...
  const sidebarItems = getAccessibleSidebarItems(currentUser)
  ...
  return (
    <aside className="w-64 my-4 ml-4 bg-white dark:bg-zinc-950 border border-slate-200 dark:border-slate-800/80 rounded-2xl flex flex-col justify-between flex-shrink-0 shadow-sm">
      ...
      <div className="m-3 p-4 bg-slate-50/60 dark:bg-zinc-900/40 border border-slate-100/80 dark:border-zinc-800/60 rounded-xl flex items-center justify-between">
        <button
          onClick={() => onSelectTab('account-admin')}
          className="flex items-center space-x-2.5 p-1.5 -m-1.5 rounded-lg hover:bg-slate-100/80 dark:hover:bg-zinc-900 transition-colors text-left focus:outline-none cursor-pointer"
          title="管理个人中心与后台"
        >
          ...
          <div>
            <p className="text-[10px] font-bold text-slate-700 dark:text-zinc-300 leading-tight">
              {currentUser?.login_name || '未登录'}
            </p>
            <span className="text-[9px] text-muted-foreground leading-none">{roleLabel}</span>
          </div>
        </button>
        <button
