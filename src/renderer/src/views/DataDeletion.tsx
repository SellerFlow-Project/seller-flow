...
export const DataDeletion: React.FC = () => {
  ...
  const [taskList, setTaskList] = useState<ScrawlTask[]>([
    { id: 'TASK-005', marketplace: '日本站 (JP)', taskType: 'Best Sellers 榜单', skuCount: 340, createdTime: '2026-05-31 17:15:30', status: 'success' },
    { id: 'TASK-004', marketplace: '美国站 (US)', taskType: 'Hot Releases 榜单', skuCount: 520, createdTime: '2026-05-31 10:42:15', status: 'success' },
    { id: 'TASK-003', marketplace: '德国站 (DE)', taskType: 'Best Sellers 榜单', skuCount: 220, createdTime: '2026-05-30 15:20:00', status: 'success' },
    { id: 'TASK-002', marketplace: '英国站 (UK)', taskType: 'Movers & Shakers', skuCount: 180, createdTime: '2026-05-29 11:10:45', status: 'failed' },
    { id: 'TASK-001', marketplace: '日本站 (JP)', taskType: 'Best Sellers 榜单', skuCount: 160, createdTime: '2026-05-28 09:30:12', status: 'success' }
  ])
  ...
  // Deletes a single specific task
  const handleDeleteSingleTask = (task: ScrawlTask) => {
    setTaskList((prev) => prev.filter((t) => t.id !== task.id))
    setDbStats((prev) => {
      const nextCount = Math.max(0, prev.amazonCount - task.skuCount)
      const sizeNum = parseFloat(prev.dbSize.replace(' MB', ''))
      const nextSize = Math.max(1.2, sizeNum - task.skuCount * 0.015).toFixed(1)
      return {
        ...prev,
        amazonCount: nextCount,
        dbSize: `${nextSize} MB`
      }
    })
    setActionLog((prev) => [
      ...
      `[成功] 成功清空任务 ${task.id} (${task.marketplace}) 及其关联的 ${task.skuCount} 条 SKU 商品细节数据。`
    ])
  }

  // Deletes all task data except the most recent one (TASK-005)
  const handleDeleteHistoryKeepLatest = () => {
    if (taskList.length <= 1) {
      setActionLog((prev) => [...prev, '[警告] 数据库中没有可清理的旧历史任务数据，无需执行此操作。'])
      return
    }
    ...
    setTaskList([latestTask])
    setDbStats((prev) => {
      const nextCount = Math.max(0, prev.amazonCount - removedSkuCount)
      const sizeNum = parseFloat(prev.dbSize.replace(' MB', ''))
      const nextSize = Math.max(1.2, sizeNum - removedSkuCount * 0.015).toFixed(1)
      return {
        ...prev,
        amazonCount: nextCount,
        dbSize: `${nextSize} MB`
  ...
  // Clears the entire database / wipe all data
  const handleClearAllData = () => {
    ...
    setActionLog((prev) => [
      ...
      `[物理擦除] 正在执行 SQLite VACUUM 磁盘空间缩减回缩动作...`,
      `[成功] 💥 全库物理清理圆满完成！累计物理擦除 ${totalRemoved} 条商品明细记录，数据库文件已缩减回缩至基准尺寸 (1.2 MB)。`
    ])
  }
    ...
    <div className="p-6 space-y-6 flex flex-col h-full overflow-y-auto bg-slate-50 dark:bg-black">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 shrink-0">
        <div className="bg-card text-card-foreground border border-border rounded-lg p-5 transition-all duration-200 hover:border-indigo-500/50 hover:shadow-sm">
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">SQLite 数据文件体积</p>
        ...
        <div className="bg-card text-card-foreground border border-border rounded-lg p-5 transition-all duration-200 hover:border-emerald-500/50 hover:shadow-sm">
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">页面缓存大小</p>
          <h3 className="text-2xl font-bold mt-1 text-foreground">{dbStats.cacheSize}</h3>
        </div>
      </div>
        ...
        <div className="lg:col-span-8 bg-card text-card-foreground border border-border rounded-lg p-6 flex flex-col justify-between transition-all duration-200 hover:border-primary/20 hover:shadow-sm h-[780px]">
          ...
          <div className="flex-1 flex flex-col min-h-0">
            ...
            <p className="text-[11px] text-muted-foreground leading-normal mb-4">
              以下展示了当前存储于本地 SQLite 数据库中的所有采集任务批次。您可以通过行尾的快捷操作一键清除对应批次下的所有商品及分类关系链数据。
            </p>
                ...
                <table className="w-full text-left border-collapse text-xs">
                  <thead>
                    <tr className="bg-slate-100/50 dark:bg-zinc-900/60 border-b border-border text-slate-500 font-semibold select-none">
                      ...
                      <th className="py-3 px-4 font-bold text-right">采集 SKU 数</th>
                      ...
                      taskList.map((task) => (
                        ...
                        >
                          ...
                          <td className="py-3.5 px-4 text-right font-black text-primary font-mono text-sm">
                            {task.skuCount} <span className="text-[10px] text-muted-foreground font-normal">SKU</span>
                          </td>
          ...
          <div className="pt-4 border-t border-border mt-4 shrink-0 flex items-center justify-between text-[10px] text-muted-foreground">
            <div className="flex items-center space-x-1.5">
              <ShieldCheck className="w-4 h-4 text-emerald-500" />
              <span>本页面所有的局部和全局清空操作均支持 SQLite ACID 完整事务原子性保证</span>
            </div>
            <span>SQLite v3.45.0 Sandbox</span>
          </div>
        </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-[10px] text-muted-foreground leading-relaxed">
                    安全清理本地存储中<b>除最新一次采集之外</b>的所有历史旧包及商品明细，常用于在多次抓取测试后释放磁盘空间。
                  </p>
                  <button
                ...
                <div className="space-y-2 pt-4 border-t border-border/80">
                  ...
                  <p className="text-[10px] text-muted-foreground leading-relaxed">
                    强制擦除数据库中的所有任务记录、商品明细，并自动执行 <b>SQLite VACUUM</b> 命令回缩文件体积。操作不可逆。
                  </p>
                  <button
                ...
                <div className="pt-2">
                  ...
                  >
                    ...
                    <span>清理系统页面临时缓存 ({dbStats.cacheSize})</span>
                  </button>
                </div>
              </div>
            </div>
          ...
          <div className="bg-card text-card-foreground border border-border rounded-lg p-6 flex flex-col transition-all duration-200 hover:border-primary/20 hover:shadow-sm flex-1 min-h-[320px]">
            <div className="flex items-center justify-between pb-4 mb-4 border-b border-border shrink-0">
              <div className="flex items-center space-x-2">
                ...
                <h2 className="font-semibold text-xs uppercase tracking-wider text-slate-500">清理事务审计日志</h2>
              </div>
              <button
            ...
            >
              {actionLog.map((log, index) => {
                ...
                if (log.startsWith('[成功]') || log.startsWith('[清理]')) color = 'text-emerald-400 font-semibold'
                if (log.startsWith('[警告]')) color = 'text-amber-400 font-semibold'
                if (log.startsWith('[物理删除]') || log.startsWith('[物理擦除]') || log.startsWith('[准备]')) color = 'text-rose-400'
                return (
